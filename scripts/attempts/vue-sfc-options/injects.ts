...
import { isStringLiteral } from '../common/is.string-literal';
import { isNumber } from '../common/is.number';
import { isBoolean } from '../common/is.boolean';
...
export const extractInjects = (scriptContent: string, absPath: string) => {
  ...
  const visitExportDefault = (node: ts.Node) => {
            ...
            for (const e of prop.initializer.properties) {
              if (ts.isPropertyAssignment(e) && ts.isIdentifier(e.name)) {
                ...
                if (e.initializer) {
                  ...
                  if (isStringLiteral(def)) type = 'string';
                  else if (isNumber(def)) type = 'number';
                  else if (isBoolean(def)) type = 'boolean';
  ...
  const visitInjectCalls = (node: ts.Node) => {
    if (
