...
import { extractEmitDescriptionFromObjectProperty } from '../common/extract-emit-description-from-object-property';
...
const handleObjectEmits = (
  ...
  for (const e of initializer.properties) {
    if (ts.isPropertyAssignment(e) && ts.isIdentifier(e.name)) {
      ...
      const description = extractEmitDescriptionFromObjectProperty(e, scriptContent);
