...
1. **Create or Update `index.ts`**  
   Each folder under `components/content/assembler/ui/<component-folder-name>/` should have an `index.ts` that exports all Vue files. For example:
...
You can check the `PatternBackground` component files `components/content/assembler/ui/pattern-background` for a complete example.
